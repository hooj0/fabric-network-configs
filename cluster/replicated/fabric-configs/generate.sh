...
function replacePrivateKey () {
    ...
    CURRENT_DIR=$PWD
    cd ./$CRYPTO_CONFIG_LOCATION/peerOrganizations/org1.hoojo.me/ca/
    PRIV_KEY=$(ls *_sk)
    cd $CURRENT_DIR

    sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" "docker-compose-fabric-${VERSION_DIR}.yaml"

    cd ./$CRYPTO_CONFIG_LOCATION/peerOrganizations/org2.hoojo.me/ca/
    PRIV_KEY=$(ls *_sk)
    cd $CURRENT_DIR
...
function fetchRequiredChannelArtifacts() {
    ...
    requiredFiles="crypto-config/peerOrganizations/org1.hoojo.me/msp/cacerts"
