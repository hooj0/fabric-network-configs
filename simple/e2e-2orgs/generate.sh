...
function replacePrivateKey () {
    ...
    echo
    echo "==> cp -r scripts/check-files.tpl.sh scripts/$VERSION_DIR/check-files.sh"
    mkdir -pv scripts/$VERSION_DIR/
	cp -rv scripts/check-files.tpl.sh scripts/$VERSION_DIR/check-files.sh

    CURRENT_DIR=$PWD
    cd ./$CRYPTO_CONFIG_OUTPUT_ROOT/peerOrganizations/org1.foo.com/ca/
    PRIV_KEY=$(ls *_sk)
    cd $CURRENT_DIR
	
    sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" scripts/$VERSION_DIR/check-files.sh 

    cd ./$CRYPTO_CONFIG_OUTPUT_ROOT/peerOrganizations/org2.bar.com/ca/
    PRIV_KEY=$(ls *_sk)
    cd $CURRENT_DIR

    sed $OPTS "s/CA2_PRIVATE_KEY/${PRIV_KEY}/g" scripts/$VERSION_DIR/check-files.sh 
    echo
}
...
function generateChannelArtifacts() {
	...
	for channel in "$@"; do
		echo "==> cryptogen -profile TwoOrgsChannel${version} -outputCreateChannelTx ./$CHANNEL_ARTIFACTS_ROOT/$channel.tx -channelID $channel"
		$CONFIGTXGEN -profile TwoOrgsChannel${version} -outputCreateChannelTx ./$CHANNEL_ARTIFACTS_ROOT/$channel.tx -channelID $channel
	done	
}
...
function cleanChannelArtifacts() {
	...
	echo "==> rm -rf ./channel-artifacts/* ./crypto-config/* ./scripts/$VERSION_DIR"
...
generateChannelArtifacts mychannel articlechannel
